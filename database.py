...
class Database:
    ...
    async def save_link(self, unique_id, message_id):
        if self.col_links is not None:
            await self.col_links.insert_one({'_id': unique_id, 'message_id': message_id, 'hf_cdn_url': None})
    ...
    async def get_hf_link(self, unique_id):
        """Returns the HF CDN URL for a file if it has been uploaded, else None."""
        if self.col_links is not None:
            doc = await self.col_links.find_one({'_id': unique_id})
            return doc.get('hf_cdn_url') if doc else None
        return None

    async def update_hf_link(self, unique_id, cdn_url):
        """Saves the HF CDN URL for a file after it has been manually or automatically uploaded."""
        if self.col_links is not None:
            await self.col_links.update_one({'_id': unique_id}, {'$set': {'hf_cdn_url': cdn_url}})
